% function process_p31_lungperfusion_xxxxxxxx
%
...
for kk = 1:1
    ...
    for jj = 1:numel(file)   
        ...
        if or(~isempty(strfind(lower(file(jj).name),'31p')), ~isempty(strfind(lower(file(jj).name),'p31')))
            ii = ii + 1;
            disp(file(jj).name)
...
%         % Select peaks and convert indicies to ppm (chemical shift)
        [p,Npyr] = max(F);
        chemshift = index2ppm(4.7,Npyr,NP,filepath); cs = cs(2:end-1);   % reference would be alfa-ATP peak
        win = 1:length(F);

        %% Correct phase and baseline. 
        ph0 = pi; ph1 = 0;
        [Fr_avg,Fc,Fr,Fi_avg] = correct_phase_and_baseline...
        (F,NP,NB,win,1,filepath,ph0,ph1,[0,1,0,1]);
        Fr_avg = Fr_avg/max(Fr_avg);

%         %% Fit spectra
%         if fit_spectra
%             if (~exist([path '/peakfit.txt'],'file'))   % If the peakfit.txt file does not exists
%                perfused_lung_13c_fit_20190807(path);
%             end
%             mets = rearrange_peaks_from_txtfile(path);
%         end
...
%         %%
        ...
        if show_in_subplot
            ...
            fig_path = data_directory;
        end
        plot(chemshift(win),Fr_avg(win)); hold off;
...
%             = integrate_peaks_and_show_kinetics_C1_pyr(path...
